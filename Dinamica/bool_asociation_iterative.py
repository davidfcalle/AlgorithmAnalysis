        ...
        for j in range( gap, n ):
            ...
            for g in range( gap ):
                ...
                if T[i][j]>count:
                    ...
                    #TT[i][k][j]=T[i][j]-count
            i = i + 1

def creatExpression(i,j,symbols,  operators):
    r=" ( "
...
def printSolutions(TT,i,j,symbols,  operators):
    ...
    for kk in range(((i*2+1)/2),len(symbols)):
        if TT[i][j][kk]>0:
            #print "=>> %i %i %i %i"%(i,kk,kk+1,j)
            leftResult=printSolutions(TT,i,kk,symbols,operators)
            rightResult=printSolutions(TT,kk+1,j,symbols,operators)
            #print "lEFT %i %i"%(i,kk)
            #print leftResult
            #print "Right %i %i"%(kk+1,j)
            #print rightResult
            if len(leftResult)==0:
                leftResult=[creatExpression(i,kk,symbols,operators)]
            if len(rightResult)==0:
                rightResult=[creatExpression(kk+1,j,symbols,operators)]
